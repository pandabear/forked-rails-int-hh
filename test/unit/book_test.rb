...
class BookTest < ActiveSupport::TestCase
  ...
  context 'tagging' do
    setup do
      @book = Factory(:book)
      @existing_untagged_tag = Factory(:tag)
    ...
    should "tag with a tag that already exists but not for this book" do
      tag = @book.tag_with(@existing_untagged_tag.name)
      assert tag
      assert_equal @existing_untagged_tag, tag
    end
    
    should "not tag with a tag that already exists, and return false" do
